...
def get_charpter_text(curl):
    ...
    if text:
        ...
        return ''
#写入文件
def write_text(name1,str1):
    path="e:/xiaoshuo";
    print(name1);
    print(str1[0])
    if not os.path.exists(path):
        os.mkdir(path);
    filePath = path+'/{}.txt'.format(name1);
    print("写入文件："+name1+"========内容："+str1[0]);
    print("开始写入");
    f = open(filePath,"w");
    f.write(str1[0]);
    print("写入结束")
    ...
    # with open(path+'{name}.txt'.format(name=name1),"w") as f:
    #     print("写入开始"+str1)
    #     f.write(str1);
    #     print("写入结束")
    #     f.close();
